...
p. Python で改行なしの出力をするには @print "foo",@ なんですね。
いつのまにか Python で解いてしまってますね・・・。

h3. Level 7

pre(prettyprint).. from PIL import Image
...
p. これで下記メッセージが表示されるのですが、この配列の意味がわかりません・・・。

bq. smart guy, you made it. the next level is [105, 10, 16, 101, 103, 14, 105, 16, 121]ljljlimkljhdfdcdfa]_a
...
p. 出てきたメッセージから答えを出します。

pre(prettyprint). print "".join(map(chr, [105, 110, 116, 101, 103, 114, 105, 116, 121]))
